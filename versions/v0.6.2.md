...
```json
{
  ...
  "$id": "https://proofofinsight.org/schema/v0.6.2/step.json",
...
**Schema `$id` path correction**

- The JSON Schema's `$id` is corrected from `https://proofofinsight.org/schemas/v0.6.2/step.json` (plural `schemas`) to `https://proofofinsight.org/schema/v0.6.2/step.json` (singular `schema`), matching the path the canonical site serves and the repository's own `schema/` directory. The change applies to both the standalone schema file and the Appendix A inline copy. No structural change to the schema; validators that resolve `$id` now succeed where they previously received 404.
